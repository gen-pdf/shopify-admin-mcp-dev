...
- `fulfillmentOrdersForMerge` `(first, after, last, before, reverse)`: `FulfillmentOrderConnection!` — Fulfillment orders eligible for merging with the given fulfillment order.
- `fulfillments` `(first, after, last, before, reverse)`: `FulfillmentConnection!` — A list of fulfillments for the fulfillment order.
- `id`: `ID!` — A globally-unique ID.
- `internationalDuties`: `FulfillmentOrderInternationalDuties` — The duties delivery method of this fulfillment order.
- `lineItems` `(first, after, last, before, reverse)`: `FulfillmentOrderLineItemConnection!` — A list of the fulfillment order's line items.
- `locationsForMove` `(lineItemIds, query, locationIds, first, after, last, before, reverse)`: `FulfillmentOrderLocationForMoveConnection!` — A list of locations that the fulfillment order can potentially move to.
- `merchantRequests` `(kind, first, after, last, before, reverse)`: `FulfillmentOrderMerchantRequestConnection!` — A list of requests sent by the merchant or an order management app to the fulfillment service for the fulfillment order.
