...
- `PRODUCT` — A product purchased, returned or exchanged.
- `TIP` — A tip added by the customer.
- `GIFT_CARD` — A gift card.
- `SHIPPING` — A shipping cost.
- `DUTY` — A duty charge.
- `ADDITIONAL_FEE` — An additional fee.
- `FEE` — A fee charge.
- `UNKNOWN` — An unknown sale line. Represents new types that may be added in future versions.
- `ADJUSTMENT` — A sale adjustment.
